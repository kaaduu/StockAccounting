...
7.  **Default Git Remote**: Prefer `origin` as the primary git remote for all operations. Always push to `origin` first, then sync with `gitea` repository when needed. The `create-release-tag.sh` script should push to `origin` by default but may also sync with `gitea`.
